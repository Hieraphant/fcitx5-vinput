...
# @description 通过豆包语音 / 火山引擎录音文件极速版接口调用云端 ASR
# @author xifan
# @version 1.0.0
# @env DOUBAO_ASR_APP_ID (required)
# @env DOUBAO_ASR_ACCESS_TOKEN (required)
# @env DOUBAO_ASR_URL (optional)
# @env DOUBAO_ASR_RESOURCE_ID (optional, default: volc.bigasr.auc_turbo)
# @env DOUBAO_ASR_MODEL_NAME (optional, default: bigmodel)
# @env DOUBAO_ASR_USER_ID (optional)
# @env DOUBAO_ASR_TIMEOUT (optional)
# ==/vinput-asr-provider==
"""Built-in Doubao / Volcengine ASR provider for vinput.

Reads one complete PCM S16_LE 16 kHz mono utterance from stdin, sends it to
the Doubao / Volcengine fast file-recognition API, and writes the final
