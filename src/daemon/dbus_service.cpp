...
static const sd_bus_vtable vtable[] = {
    ...
    SD_BUS_METHOD("StartRecording", "", "",
                  &DbusService::handleStartRecording,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("StopRecording", "s", "s",
                  &DbusService::handleStopRecording,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("GetStatus", "", "s",
...
bool DbusService::Start() {
    ...
    ret = sd_bus_add_object_vtable(bus_, &slot_, kObjectPath, kInterface,
                                    vtable, this);
    if (ret < 0) {
