...
std::vector<std::string> ExtractStructuredCandidates(
...
std::optional<std::vector<std::string>> RewriteWithOpenAiCompatible(
    const std::string& text, const vinput::scene::Definition& scene,
    const VinputSettings& settings, int candidate_count) {
    if (!settings.llmEnabled || !scene.llm) {
        return std::nullopt;
    }
    if (settings.llmBaseUrl.empty() || settings.llmModel.empty()) {
        fprintf(stderr,
                "vinput-daemon: LLM postprocess enabled but base URL/model is "
                "missing\n");
        return std::nullopt;
    }
    ...
    if (!settings.llmApiKey.empty()) {
        const std::string auth = "Authorization: Bearer " + settings.llmApiKey;
    ...
    curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS, settings.llmTimeoutMs);
...
vinput::result::Payload PostProcessor::Process(
    const std::string& raw_text, const vinput::scene::Definition& scene,
    ...
    const int candidate_count =
        NormalizeCandidateCount(settings.llmCandidateCount);
    auto rewritten =
