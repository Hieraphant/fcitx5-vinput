...
int main(int argc, char* argv[]) {
    ...
    const auto model_paths = model_mgr.GetPaths();
    ...
    if (!disable_asr) {
        fprintf(stderr,
                "vinput-daemon: using model '%s' from %s\n",
                model_mgr.GetModelName().c_str(), model_paths.model.c_str());
        fprintf(stderr, "vinput-daemon: using tokens from %s\n",
                model_paths.tokens.c_str());
    }

    AsrEngine asr;
    if (!disable_asr && !asr.Init(model_paths, 4)) {
    ...
    dbus.SetStopHandler([&](const std::string& scene_id) -> std::string {
        ...
        if (pcm.size() < AsrEngine::kMinSamplesForInference) {
            fprintf(stderr,
                    ...
                    "%zu samples (%.1f ms)\n",
                    pcm.size(),
                    static_cast<double>(pcm.size()) * 1000.0 / 16000.0);
            current_status = Status::Idle;
            dbus.EmitStatusChanged(StatusToString(Status::Idle));
            return "";
        }
        ...
        if (!text.empty()) {
            ...
            if (runtime_settings.llmEnabled && scene.llm) {
                ...
                dbus.EmitStatusChanged(
                    StatusToString(Status::Postprocessing));
            }
    ...
    dbus.SetStatusHandler([&]() -> std::string {
        return StatusToString(current_status.load());
    });
