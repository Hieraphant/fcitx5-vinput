...
void VinputEngine::setupDBusWatcher() {
    ...
    fcitx::dbus::MatchRule result_rule(
        kBusName, kObjectPath, kInterface, kSignalRecognitionResult);

    result_slot_ = bus_->addMatch(
        result_rule,
    ...
    fcitx::dbus::MatchRule status_rule(
        kBusName, kObjectPath, kInterface, kSignalStatusChanged);

    status_slot_ = bus_->addMatch(
        status_rule,
        [this](fcitx::dbus::Message& msg) {
            onStatusChanged(msg);
            return true;
...
void VinputEngine::keyEvent(const fcitx::InputMethodEntry& entry,
...
void VinputEngine::activate(const fcitx::InputMethodEntry& entry,
                             fcitx::InputContextEvent& event) {
    (void)entry;
    active_ic_ = event.inputContext();
}

void VinputEngine::deactivate(const fcitx::InputMethodEntry& entry,
                              fcitx::InputContextEvent& event) {
    (void)entry;
    (void)event;
    hideResultMenu();
...
bool VinputEngine::handleResultMenuKeyEvent(fcitx::KeyEvent& keyEvent) {
    ...
    if (MatchesKeyList(keyEvent.key(), page_prev_keys_)) {
        ChangeCandidatePage(result_menu_ic_,
                            ResultMenuTitle(result_candidates_.size()),
                            false);
        keyEvent.filterAndAccept();
        return true;
    }
...
void VinputEngine::restartDaemon() {
    ...
    auto msg = bus_->createMethodCall(kSystemdBusName, kSystemdPath,
    ...
    if (reply.isError()) {
        fprintf(stderr,
                "vinput: systemd restart failed: %s: %s\n",
...
fcitx::AddonInstance* VinputEngineFactory::create(
