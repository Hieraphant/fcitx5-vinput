...
inline const char* StatusToString(Status s) {
    switch (s) {
        ...
        case Status::Inferring: return "inferring";
        case Status::Postprocessing: return "postprocessing";
...
inline Status StringToStatus(const std::string& s) {
    ...
    if (s == "recording") return Status::Recording;
    if (s == "inferring") return Status::Inferring;
    if (s == "postprocessing") return Status::Postprocessing;
