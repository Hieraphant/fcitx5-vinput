...
VOSK_VERSION="${VOSK_VERSION:-0.3.50}"
...
VOSK_RUNTIME_TARGET_DEFAULT="${VOSK_RUNTIME_TARGET_DEFAULT:-linux-x86_64}"

vosk_runtime_set_vars() {
    local target="${1:-${VOSK_RUNTIME_TARGET_DEFAULT}}"
    local kaldi_short="${KALDI_VOSK_REV:0:12}"
    local openfst_short="${OPENFST_VOSK_REV:0:12}"

    case "${target}" in
        linux-x86_64|ubuntu24.04|debian12|opensuse-leap|flatpak-kde6.10)
            ;;
        *)
    ...
    case "${target}" in
        ubuntu24.04|debian12|opensuse-leap|flatpak-kde6.10)
            target="${VOSK_RUNTIME_TARGET_DEFAULT}"
            ;;
    esac
