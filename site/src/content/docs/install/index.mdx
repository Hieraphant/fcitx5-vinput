...
Source builds default to the Fcitx5 system prefix (`/usr`) so the addon is installed under the directories Fcitx5 scans. If you installed an older build under `/usr/local`, reinstall from a clean build directory so `vinput.conf` and `fcitx5-vinput.so` move to the Fcitx5 system paths.
